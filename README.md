...
- **影片與動圖先外包給看得了片的模型。** 主模型收不下影片本身，抽格又只看得到
  幾個瞬間 —— 連續動作、節奏、字幕變化都看不到。所以整段片交給
  `FW_VIDEO_DELEGATE_MODEL`（預設 `google/gemini-3.5-flash-lite`）看完，
  拿一段文字解說回來。**成功就不必再送圖**，失敗才退回抽格。
  解說會落庫成為對話紀錄的一部分，所以下一輪它還記得那條片在做什麼
- **外包有兩個上限**：長度（`FW_VIDEO_DELEGATE_MAX_SECONDS`，預設 300 秒）與
  大小（`FW_VIDEO_DELEGATE_MAX_BYTES`，預設 10 MB）。影片輸入**按秒計費**，
  300 秒約 $0.023，而抽格只要約 $0.0004 —— 所以長度上限就是成本槓桿。
  大小上限往往更早觸發（5 分鐘的 720p 片通常遠超 10 MB），所以兩種原因
  分開講
- **超過上限會講出來，不是靜靜退回抽格。** 否則對方會以為整段都被看過了，
  而實際上只看得到幾格。訊息會進提示，讓它照實講（人設那邊有對應說明）
- 抽格那條路（外包退回時用）：三格靜態畫面，中段均勻取樣（避開開頭的
  標題卡與結尾的 logo）
