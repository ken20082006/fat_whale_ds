...
- **影片與動圖外包給看得了片的模型。** 主模型收不下影片本身，抽格又只看得到
  幾個瞬間 —— 連續動作、節奏、字幕變化都看不到。所以整段片交給
  `FW_VIDEO_DELEGATE_MODEL`（預設 `google/gemini-3.5-flash-lite`）看完，
  拿一段文字解說回來。解說會落庫成為對話紀錄的一部分，所以下一輪它還記得
  那條片在做什麼
- **睇唔到就直接唔睇。** 外包失敗就當作沒有這個媒體，並把原因講出來 ——
  **不再抽幾格充數**。一個定格看不出連續動作，卻會讓模型以為自己看過，
  然後講出半真半假的描述（這真的發生過，而且聽起來像真的）。人設那邊
  也寫明要照實講看不到
- **引用串裡的影片也外包。** `group_cache` 存了本體（`clip_file_id` /
  `clip_seconds` / `clip_bytes`），所以「先傳 GIF、再回覆它問問題」這種
...
- **外包有兩個上限**：長度（`FW_VIDEO_DELEGATE_MAX_SECONDS`，預設 300 秒）與
  ...
  所以兩種原因分開講
- `FW_VIDEO_DELEGATE_ENABLED=false` 會**回到舊的抽格行為** —— 否則「關掉
  外包」會變成「影片完全隱形」，那不是任何人想要的
- 抽格那條路（只在上面那個開關關掉時用）：三格靜態畫面，中段均勻取樣
  （避開開頭的標題卡與結尾的 logo）。**GIF 走 Pillow 逐格抽**、ffmpeg 由
  `imageio-ffmpeg` 自帶 —— 這兩條也只在抽格那條路才用得到
