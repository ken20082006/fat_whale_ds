...
> **已知限制：外掛每次請求只搜一次，而且查詢由引擎自己從對話推導 ——
> 模型無權指定。** 所以在這條路徑下「搜唔到小眾名詞」是預期之內的事，
> 不是 bug（實際案例：問某個型號幾時開賣，引擎撈到的是主流型號，那個
> 小眾名字從未出現在五條結果裡，而沒有第二次嘗試）。
>
> 唯一能調召回率的旋鈕是**撈幾多條結果**，現在由判斷逐則決定
> （`FW_SEARCH_RESULTS_QUICK` / `FW_SEARCH_MAX_RESULTS` /
> `FW_SEARCH_RESULTS_THOROUGH`）。
>
> 結構性的解法在 `TODO.md`：改用伺服器端工具之後，模型可以自己決定搜幾次、
> 用什麼查詢（實測一次請求會搜 2–4 次）。
...
**判斷會帶上上一輪對話。** 短追問單獨看會失真：「iphone duo喎」單獨看只是
一個三個字的片段，判斷成「直接答」（信心 0.81）；帶上「上一句問緊開賣日期」
之後才會判斷成「要搜尋」。沒有上文的話，使用者追問一次就等於白問。
