...
長期筆記有 `scope` 欄位，私聊是 `private`，每個群組是 `group:<chat_id>`。

**兩者完全不互通。** 這不是潔癖，是隱私要求：如果共用同一份筆記，朋友在私聊說過
「我最近失業」，助理可能在群組裡脫口說出來。分開存之後，私聊記的事永遠不會在群組出現。

同一個人在不同群組也各自獨立 —— 你在 A 群講的事不會被帶到 B 群。
