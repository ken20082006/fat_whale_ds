...
- 完整引用鏈回溯：引用再引用也追得到源頭
- **近期發言也算背景**：引用鏈只追得到互相引用的那一串。同一桌其他人如果
  沒有互相引用，助理會另外參考最近的發言（`FW_GROUP_RECENT_MESSAGES`，
  預設 10 則，與引用串重疊的會剔除）—— 甲貼了張咖啡相、乙跟著也貼一張
  問評價時，它才答得出「甲也貼過」。這些只是背景，不是要它逐則回應
...
```
人設 + 對象 + 長期筆記 + 摘要 + 群組近期發言 → 最近原文 → 新訊息
```

（群組近期發言只出現在群組，見「群組」一節。）
...
2. 保留 72 小時後自動清除
3. **送進模型的只有兩類**：被指名的那條引用串，以及最近的
   `FW_GROUP_RECENT_MESSAGES` 則（預設 10，與引用串重疊的會剔除）。
   其餘內容永不送進模型
