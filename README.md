...
- **影片與動圖一律外包給看得了片的模型。** 主模型只看得懂靜態圖，而抽幾個
  定格看不出連續動作、節奏與字幕變化，還會讓它以為自己看過整段。所以整段片
  交給 `FW_VIDEO_DELEGATE_MODEL`（預設 `google/gemini-3.5-flash-lite`）看完，
  拿一段文字解說回來。解說會落庫成為對話紀錄的一部分，所以下一輪它還記得
  那條片在做什麼
- **所以本專案不需要 ffmpeg。** 沒有抽格這回事，也就不必在本機解碼影片。
- **睇唔到就直接唔睇。** 外包失敗就當作沒有這個媒體，並把原因講出來 ——
  **不抽格充數**。人設那邊也寫明要照實講看不到
- **解說按媒體的穩定識別碼快取。** 這不只是省錢，也是為了一致：同一條片
  再傳，外包模型會給出唔同的描述（實測同一條 GIF 三次得到「鯨魚噴水」
  「掀檯」「街頭窄巷」）。**貼圖尤其是重點** —— 同一張會反覆出現，而貼圖的
  `file_unique_id` 永久穩定，所以睇一次就夠，之後免費
- **引用串裡的影片也外包。** `group_cache` 存了本體（`clip_file_id` /
  `clip_seconds` / `clip_bytes`），所以「先傳某條片、再回覆它問問題」這種
  常見用法走得通。一輪最多外包 `FW_VIDEO_DELEGATE_CHAIN_LIMIT` 條（預設 1）——
  每條最貴約兩仙美元，一串四條片就是 $0.09，不該默默發生
- **外包有兩個上限**：長度（`FW_VIDEO_DELEGATE_MAX_SECONDS`，預設 300 秒）與
  大小（`FW_VIDEO_DELEGATE_MAX_BYTES`，預設 10 MB）。影片輸入**按秒計費**，
  300 秒約 $0.023。大小上限往往更早觸發（5 分鐘的 720p 片通常遠超 10 MB），
  所以兩種原因分開講
- **動態貼圖（`.tgs`）是例外。** 它是 Lottie JSON，不是影片 —— 外包看唔到，
  只有 Telegram 附的那張靜態縮圖可用，所以走一般圖片那條路。
  **影片貼紙（`.webm`）就真的是影片**，走外包而且會快取
