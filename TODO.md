...
## 已完成（2026-09-24）：回答準確性 —— 聯網搜尋改用伺服器端工具

**症狀**：即使開了聯網，面對世界知識類問題（新聞、版本、價格、日期）仍經常
不查就直接回答，內容憑印象編造。

**結果**：已改用伺服器端工具 `openrouter:web_search`。實測一次請求會搜
**2–4 次**、查詢由模型自己決定、拿到的是原文片段而非引擎摘要。

**與原計劃不同之處**（全是探測之後才發現的，用 `scripts/ping.py --search`
可以重新驗證每一點）：

1. **`usage.server_tool_use` 不存在。** 實際欄位是
   `usage.server_tool_use_details`。照原本寫法會永遠讀到 0，而且是靜默失效。
2. **`content` 是字串**，不是陣列 —— 原本擔心的 `AttributeError` 不存在。
   （`_read_content()` 仍然兩種都吃，因為文件兩種都寫過。）
3. **`url_citation` 的 `start_index` / `end_index` 全部是 0。** 引用沒有錨定
   在正文位置，所以做不到「標在對應句子上」，最多只能附一份來源清單。
4. **`usage.cost` 已含搜尋費**，而 `cost_details.upstream_inference_cost` 是
   當中的推論部分。相減就精確得到搜尋費，不必另外估。
5. **regex 閘沒有刪。** 它降級成後備（判斷失手時的降級路徑）—— 刪掉就沒有
   那條路了。Jev 已經接手主要判斷。
6. **`[[搜尋:...]]` 標記機制已刪。** 伺服器端工具本身就是「讓模型自己決定」，
   標記只是它的替代品；而且它會產生「整則回覆只有標記、清完變空白」的問題。
7. 順帶發現：帶工具時模型會先講一句英文旁白（`I'll search for…`）才回答。
   已在 persona 明講「查到就直接答，不要先講我要去查」。

### 原本的根因分析（保留作紀錄 —— 三個閘都會漏）
...
- ~~`settings.py` 的預設 `model` 仍是 `deepseek/deepseek-chat`~~ —— **已修**
  （2026-09-24）：三個模型預設都改成 `deepseek/deepseek-v4.1-flash`。
  舊預設不吃圖，`.env` 一旦遺失或換機重建，視覺能力會靜靜失效。
- `FW_MODEL_VISION` 若指向不吃圖的模型，圖片路徑會失效。**仍未處理** ——
  沒有可靠的方法在啟動時判斷某個模型吃不吃圖（`/models` 的欄位不足以判斷），
  只能靠人記得。已在 `.env.example` 標明「必須具備視覺能力」。
- **引用串裡的影片沒辦法外包看片。** `group_cache` 只存了 `media_file_id`，
  而 `pick_file()` 對影片回傳的是**縮圖**的 file_id，不是本體。所以要重看
  一條早前訊息裡的影片時，只有一格縮圖可用，外包那條路取不到片。
  要修的話得在 `group_cache` 多存一個欄位（本體的 file_id 與長度），並在
  快取時一併填。**目前直接傳來的影片不受影響**（那條路拿得到本體），
  只有「別人引用一條舊片」的情境會降級成縮圖。
