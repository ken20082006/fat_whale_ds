...
- ~~引用串裡的影片沒辦法外包看片~~ —— **已修**（2026-09-24）：
  `group_cache` 加 `clip_file_id` / `clip_seconds` / `clip_bytes` 三欄，
  快取時一併填，`_gather_chain_notes()` 用它外包。這條限制當日就撞到
  一次（使用者先傳 GIF、再回覆它問問題，於是媒體只在引用串裡），
  而且表現係最壞嘅一種：模型講出半真半假嘅描述，聽起來像真嘅。
