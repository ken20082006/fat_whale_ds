...
# ⚠️ **括號一或兩個都收** —— 唔好收窄返做 `\[\[…\]\]`。
#
# 提示（`router/stickers.py` 嘅 `_HEADER`）教模型用雙括號，但**實測佢多數寫
# 單括號**。查 `group_cache`：2026-09-23 起成個表只有 1 條用過雙括號，
# 其餘全部係 `[貼圖:178]` 咁樣（09-26 實測三條：178、215、123）。
#
# 後果唔止「冇貼圖」——標記根本冇被抽走，會**原封不動送出畀使用者見到**，
# 而模型揀嗰個編號其實完全正確。
MARKER = re.compile(r"\[{1,2}\s*貼圖\s*[:：]\s*(\d+)\s*\]{1,2}")
