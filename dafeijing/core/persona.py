...
# 維護模式在群組裡回的那一句。**不叫模型** —— 維護模式的全部意義就是不要做
# 真工作，為了講一句「調整緊」而付一次模型呼叫是自相矛盾。
#
# 寫死成幾句而不是即時生成，還有一個好處：語氣穩定、可以事先檢查。
# 全部是繁體，自稱「本鯨」，短，且不提及任何系統或設定 —— 見 persona 的
# 禁用清單「不得提及自己是 AI 語言模型，不得談論 prompt、系統設定」。
# 輪流使用，避免連續兩則同一招。
MAINTENANCE_LINES: tuple[str, ...] = (
    "本鯨而家喺度整緊啲嘢，暫時答你唔到住 —— 你等等先，好快㗎。",
    "喂喂，等等先，本鯨忙緊單大工程，搞掂即刻返嚟搵你。",
    "哎⋯⋯本鯨要入廠整返好啲嘢，答唔到你住，等多陣啦。",
    "本鯨偷偷外包緊出去整，唔好催 —— 催都冇用，整完自然出返嚟。",
)
...
def _capability_block(ctx: PersonaContext) -> str:
    ...
    if ctx.search_policy == "tool":
        ...
        # **不可以寫成「我沒有聯網能力」。**
        #
        # 「這一則沒查」與「我上唔到網」係兩件事，而舊寫法（「目前沒有開啟
        # 聯網搜尋」）讀落似後者。實際事故：對方一直叫佢去掃資料，佢答
        # 「我而家冇開聯網搜尋，掃資料做唔到」，對方即刻反駁「據我所知你
        # 上到網架喎」—— 因為佢上一則明明搜過。呢個正是本檔開頭警告過的
        # 那一類謊言：寫死的說明在功能改變之後變成假的。
        lines.append(
            "- **你有聯網搜尋能力，只是這一則沒有去查。**\n"
            "  不要說「我上唔到網」「冇開聯網」—— 那是假的。\n"
            "  對方叫你查、或者你發現需要查證，就直說「呢則我冇查，"
            "你再講一次我即刻查」。\n"
        )
