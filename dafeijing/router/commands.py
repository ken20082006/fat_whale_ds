...
**大部分由舊版搬過嚟。** 原本係 `from .telegram import …` 借返嚟，
但咁樣會令 `bot/commands.py` 連帶 import 成個舊腦（`core/chat` 等），
而 Router 一行都冇叫過 —— 依賴分析睇得出，但 import 鏈拖住。

搬入嚟之後 `bot/` 可以整個刪走，`core/chat.py`、`core/security.py`、
`core/webfetch.py` 亦跟住死。

**冇搬嘅指令**（Router 唔支援）：`/tune`、`/reload_persona`、`/undo`、
`/export`、`/vibe`、`/think`、`/search` —— 嗰啲控制舊版自己個腦，
...
async def _scope_label(svc: RouterServices, scope: str) -> str:
...
async def cmd_stats(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    from .telegram import admin_only, get_services
